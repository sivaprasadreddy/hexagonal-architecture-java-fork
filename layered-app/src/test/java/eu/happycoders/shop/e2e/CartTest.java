...
import static eu.happycoders.shop.rest.cart.CartsControllerAssertions.assertThatResponseIsCart;
import static eu.happycoders.shop.persistence.DemoProducts.LED_LIGHTS;
...
class CartTest {
  ...
  void givenAnEmptyCart_addLineItem_addsTheLineItemAndReturnsTheCartWithTheAddedItem()
    ...
    Response response =
        given()
            .port(port)
            .queryParam("productId", LED_LIGHTS.getId().value())
  ...
  void givenACartWithOneLineItem_addLineItem_addsTheLineItemAndReturnsACartWithTwoLineItems()
    ...
    Response response =
        given()
            .port(port)
            .queryParam("productId", MONITOR_DESK_MOUNT.getId().value())
